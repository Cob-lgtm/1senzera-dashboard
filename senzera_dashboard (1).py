...
def load_erstgaeste(path: str = "Erstgaeste_Data.csv") -> pd.DataFrame:
    if not os.path.exists(path):
        return pd.DataFrame()
    df = pd.read_csv(path, sep=";")
    if df.empty or "Erstgaeste" not in df.columns:
        return pd.DataFrame()
    # Add Gesamt row per Monat
    gesamt = df.groupby("Monat", as_index=False)["Erstgaeste"].sum()
    gesamt["Regionalleitung"] = "Gesamt"
    return pd.concat([df, gesamt], ignore_index=True)
...
if not df_erstgaeste.empty:
    ...
    eg_sel = eg_monat[eg_monat["Regionalleitung"] == "Gesamt"]
    ...
    if not eg_monat[eg_monat["Regionalleitung"] != "Gesamt"].empty:
        eg_sorted = eg_monat[eg_monat["Regionalleitung"] != "Gesamt"].sort_values("Erstgaeste", ascending=False)
...
with tab1:
    ...
    if not df_erstgaeste.empty:
        eg_tab = df_erstgaeste[(df_erstgaeste["Monat"] == sel_monat) & (df_erstgaeste["Regionalleitung"] != "Gesamt")].copy()
        if not eg_tab.empty:
            st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)
            section_title("Erstgäste nach Region", f"Neukunden · {sel_monat}")
            eg_tab = eg_tab.sort_values("Erstgaeste", ascending=True)
            fig_eg = go.Figure(go.Bar(x=eg_tab["Erstgaeste"], y=eg_tab["Regionalleitung"], orientation="h", marker=dict(color=C_ORANGE, line=dict(width=0)), text=eg_tab["Erstgaeste"], textposition="outside", textfont=dict(color=PFG, size=12), hovertemplate="<b>%{y}</b><br>%{x} Erstgäste<extra></extra>"))
            fig_eg.update_layout(**plotly_base(), xaxis=dict(gridcolor=PGRD, zeroline=False, tickfont=dict(size=11)), yaxis=dict(tickfont=dict(size=11, color=T["text_second"])), margin=dict(l=4, r=55, t=8, b=8), height=max(200, len(eg_tab) * 36), bargap=0.38)
...
def load_erstgaeste(path: str = "Erstgaeste_Data.csv") -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        parts = str(r.get("Segment", "")).split()
        if len(parts) >= 4 and parts[0] == "Erstgaeste":
            monat_raw = parts[1]
            year = parts[2]
            region = parts[3] if len(parts) > 3 else "Gesamt"
            _monat_fix = {"Januar": "Januar", "Februar": "Februar", "Maerz": "M\u00e4rz",
            ...
            monat_label = f"{_monat_fix.get(monat_raw, monat_raw)} {year}"
            rows.append({"Monat": monat_label, "Regionalleitung": region,
                         "Erstgaeste": int(r.get("Size Estimate", 0))})
