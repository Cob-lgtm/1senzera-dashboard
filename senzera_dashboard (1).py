"""
Senzera Performance Hub â v5 (Mobile Optimized)
============================================
...
RATING_MIN  = 4.2
RATING_GOOD = 4.5
REQ_GOOGLE  = {"StudiokÃ¼rzel", "Stadt", "Regionalleitung", "Rating"}
REQ_ZEN     = {"Property - studio", "score_type", "score"}

# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
# 3 Â· THEME DEFINITIONEN
# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
THEMES = {
    "âï¸ Hell": {
        "mode": "light",
...
if "theme" not in st.session_state:
    st.session_state.theme = "âï¸ Hell"
...
/* WICHTIG: Hamburger-MenÃ¼ auf Handys sichtbar machen! */
...
# 5 Â· DATEN LADEN
# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
@st.cache_data(show_spinner="ð¸  Daten werden geladen â¦")
def load_google(path: str = "Senzera_Dashboard_Data.csv") -> pd.DataFrame:
    ...
    df["Studio_Name"]  = df["StudiokÃ¼rzel"] + " (" + df["Stadt"] + ")"
...
if df_google.empty:
    st.error("â **'Senzera_Dashboard_Data.csv'** nicht gefunden.")
    st.stop()
...
def rating_icon(r: float) -> str:
    return "â" if r >= RATING_GOOD else ("â ï¸" if r >= RATING_MIN else "ð¨")

def nps_label(n: float) -> str:
    if n >= 70: return "Weltklasse ð"
    if n >= 50: return "Exzellent ð"
    if n >= 30: return "Gut ð"
    if n >= 0:  return "AusbaufÃ¤hig â ï¸"
    return "Kritisch ð¨"
        ...
        f"""<div style='display:flex;align-items:center;gap:10px;margin:5px 0;padding:9px 13px;background:{bg_};border:1px solid {T["card_border"]};border-radius:10px;'>
            <div style='flex:1;min-width:0;'>
                <div style='display:flex;justify-content:space-between;margin-bottom:5px;'>
                    ...
                    <span style='font-size:11.5px;font-weight:700;color:{color};'>{cnt}Ã</span>
                </div>
...
with st.sidebar:
    ...
    if len(all_months) > 1:
        ...
        st.markdown(f"<div style='padding:10px 12px;background:{T['card_bg']};border:1px solid {T['card_border']};border-radius:11px;font-size:13px;color:{T['text_body']};margin-bottom:12px;'>ð <b>{sel_monat}</b></div>", unsafe_allow_html=True)
    ...
    if not sel_studios:
        st.warning("â ï¸ Bitte mindestens ein Studio wÃ¤hlen.")
        st.stop()
    ...
    if st.button("ð Cache leeren & neu laden", use_container_width=True):
        st.cache_data.clear()
        st.rerun()
...
sel_codes = df_curr["StudiokÃ¼rzel"].unique()
...
with hc1:
    ...
    vgl_html = f"&nbsp;Â·&nbsp; Vergleich zu <b>{vormonat}</b>" if vormonat else ""
    st.markdown(
...
with hc2:
    if n_crit > 0: st.error(f"ð¨ {n_crit} kritisch")
    else: st.success("â Alles OK")

# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
# 10 Â· KPIs (3 + 3 Layout)
# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
k1, k2, k3 = st.columns(3)
k1.metric("Google Ã-Rating", f"{avg_rating:.2f} â­", delta=f"{delta_r:+.2f} vs. {vormonat}" if delta_r is not None else None, delta_color="normal" if delta_r and delta_r >= 0 else "inverse")
k2.metric("Neue Rezensionen", f"{new_rev:,}".replace(",", "."), delta=sel_monat, delta_color="off")
k3.metric("Rezensionen Gesamt", f"{total_rev:,}".replace(",", "."), delta="alle Monate", delta_color="off", help="Summe aller verfÃ¼gbaren Google-Rezensionen der gewÃ¤hlten Studios")
...
if nps_ges is not None:
    ...
    k4.metric("Zenloop NPS", "â", delta="keine Daten", delta_color="off")
k5.metric("Positive Stimmung", f"{sentiment:.0f}%" if sentiment is not None else "â", delta="der Kommentare", delta_color="off", help="Anteil positiver Kommentare")
...
if not crit.empty:
    st.markdown(
        f"""<div style='background:linear-gradient(135deg,rgba(200,48,16,0.07),rgba(200,48,16,0.03));border:1px solid rgba(200,48,16,0.2);border-left:4px solid {C_RED};border-radius:14px;padding:14px 20px;margin-bottom:16px;display:flex;align-items:center;gap:12px;'>
            <div style='font-size:22px;'>ð¨</div>
    ...
    for idx, (_, row) in enumerate(crit.iterrows()):
        with alarm_cols[idx % 4]:
            zs = df_zen[df_zen["Property - studio"] == row["StudiokÃ¼rzel"]] if not df_zen.empty else pd.DataFrame()
            sn = get_nps(zs)
            nl = neg_labels(zs, 2)
            nstr = f"NPS: <b>{sn:.0f}</b>" if sn is not None else ""
            nlstr = (f"<div style='margin-top:6px;font-size:11px;color:{C_RED};'>â ï¸ {chr(160).join(nl.index.tolist())}</div>") if not nl.empty else ""
            ...
            st.markdown(
                f"""<div style='background:{T["card_bg"]};border:1px solid rgba(200,48,16,0.15);border-top:3px solid {r_color};border-radius:14px;padding:16px 18px;margin-bottom:4px;box-shadow:{T["card_shadow"]};'>
                    <div style='font-size:15px;font-weight:700;color:{T["text_h"]};margin-bottom:2px;'>{row["StudiokÃ¼rzel"]}</div>
                    ...
                    <div style='display:flex;gap:8px;flex-wrap:wrap;align-items:center;'>
                        ...
                        <span style='font-size:11px;color:{T["text_muted"]};padding-top:3px;'>â­ Â· +{row["NewReviews"]} Rez.</span>
                        {f'<span style="font-size:11px;color:{T["text_muted"]};padding-top:3px;">Â· {nstr}</span>' if nstr else ""}
    ...
    st.markdown(
        f"""<div style='background:{T["ok_bg"]};border:1px solid rgba(58,138,80,0.2);border-left:4px solid {C_MINT};border-radius:14px;padding:14px 20px;margin-bottom:16px;display:flex;align-items:center;gap:12px;'>
            ...
            <div><div style='font-size:12px;font-weight:800;letter-spacing:1px;text-transform:uppercase;color:{C_GREEN};margin-bottom:2px;'>Alles im grÃ¼nen Bereich</div>
            <div style='font-size:13px;color:{T["text_h"]};'>Alle {n_sel} Studios Ã¼ber {RATING_MIN} Sternen</div></div>
        </div>""", unsafe_allow_html=True)
...
# 12 Â· TABS
# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
tab1, tab2, tab3, tab4 = st.tabs(["  ð  Performance & Trends  ", "  ð  Zenloop Deep-Dive  ", "  ð  Management-Bericht  ", "  ð  Team-Performer  "])

# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
# TAB 1 Â· PERFORMANCE & TRENDS
# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
with tab1:
    ...
    with col_l:
        ...
        fig_rk = go.Figure(go.Bar(x=df_rk["Rating"], y=df_rk["StudiokÃ¼rzel"], orientation="h", marker=dict(color=bar_c, line=dict(width=0)), text=df_rk["Rating"].apply(lambda r: f"{r:.2f}"), textposition="outside", textfont=dict(color=PFG, size=12), hovertemplate="<b>%{y}</b><br>â­ %{x:.2f}<extra></extra>"))
    ...
    with col_r:
        section_title("Trend-Verlauf", "Monatlicher Ã-Rating")
        trend = df_view.groupby("Monat", sort=False)["Rating"].mean().reset_index().rename(columns={"Rating": "Ã Rating"})
        fig_tr = go.Figure()
        fig_tr.add_trace(go.Scatter(x=trend["Monat"], y=trend["Ã Rating"], fill="tozeroy", fillcolor="rgba(232,98,10,0.07)", line=dict(width=0), showlegend=False, hoverinfo="skip"))
        fig_tr.add_trace(go.Scatter(x=trend["Monat"], y=trend["Ã Rating"], line=dict(color=C_ORANGE, width=2.5), marker=dict(size=8, color=C_ORANGE, line=dict(width=2.5, color=T["app_bg"])), mode="lines+markers", hovertemplate="<b>%{x}</b><br>Ã Rating: %{y:.2f}<extra></extra>"))
        y_min = max(3.0, trend["Ã Rating"].min() - 0.4)
        y_max = min(5.1, trend["Ã Rating"].max() + 0.3)
    ...
    st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)
    section_title("Studio-Ãbersicht", f"Sortiert nach Rating Â· {sel_monat}")
    ...
    if not df_vm.empty:
        vm_map = df_vm.set_index("StudiokÃ¼rzel")["Rating"].to_dict()
        def row_delta(row):
            m = df_curr[df_curr["Studio_Name"] == row.get("Studio_Name", "")]["StudiokÃ¼rzel"]
            if m.empty: return ""
            old = vm_map.get(m.values[0])
            if old is None: return ""
            dv = round(row["Rating"] - old, 2)
            return f"+{dv}" if dv > 0 else str(dv)
        df_d["Î Vormonat"] = df_d.apply(row_delta, axis=1)
    df_d.columns = [c.replace("_", " ") for c in df_d.columns]
    st.dataframe(df_d, use_container_width=True, hide_index=True)


# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
# TAB 2 Â· ZENLOOP DEEP-DIVE
# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
with tab2:
    if df_zen.empty:
        st.warning("â ï¸ Keine Zenloop-Daten. Bitte 'Zenloop_Antworten.csv' bereitstellen.")
    else:
        pc1, pc2 = st.columns([1, 3])
        with pc1:
            section_title("Studio-Check")
            sel_s = st.selectbox("Studio wÃ¤hlen", codes_zen, label_visibility="collapsed")
        ...
        m1.metric("NPS", f"{s_nps:.0f}" if s_nps is not None else "â", delta=nps_label(s_nps) if s_nps is not None else None, delta_color="normal" if s_nps and s_nps >= 0 else "inverse")
        m2.metric("Promoter ð", str(s_prom))
        m3.metric("Passive ð", str(s_pass))
        m4.metric("Detraktoren ð ", str(s_detr))

        st.markdown("<div style='height:14px'></div>", unsafe_allow_html=True)
        lc, rc = st.columns(2, gap="large")
        with lc:
            section_title("Top-Themen", "HÃ¤ufigkeit im Feedback")
            ...
            if not nlb.empty:
                ...
                section_title("â ï¸ Kritikpunkte", "Themen der Detraktoren")
                ...
                for _, row in df_com.iterrows():
                    ...
                    st.markdown(
                        f"""<div style='padding:13px 15px;background:{T["card_bg"]};border:1px solid {T["card_border"]};border-radius:13px;margin-bottom:9px;'>
                            ...
                            <div style='font-size:12.5px;color:{T["text_second"]};line-height:1.6;font-style:italic;'>â{str(row["comment"])[:200]}{"â¦" if len(str(row["comment"])) > 200 else ""}"</div>
                        </div>""", unsafe_allow_html=True)

        st.divider()
        section_title("Regionale Ãbersicht", "Alle ausgewÃ¤hlten Studios")
        ov1, ov2 = st.columns(2, gap="large")
        with ov1:
            st.markdown(f"<div style='font-size:13px;font-weight:600;color:{T['text_h']};margin-bottom:12px;'>HÃ¤ufigste Themen gesamt</div>", unsafe_allow_html=True)
            ...
            if not all_lb.empty:
                ...
                fig_lb = go.Figure(go.Bar(x=all_lb["Anzahl"], y=all_lb["Thema"], orientation="h", marker=dict(color=ci[:len(all_lb)], line=dict(width=0)), text=all_lb["Anzahl"], textposition="outside", textfont=dict(color=PFG, size=11), hovertemplate="<b>%{y}</b><br>%{x}Ã<extra></extra>"))
        ...
        if "score_type" in df_zen.columns:
            st.markdown("<div style='height:6px'></div>", unsafe_allow_html=True)
            section_title("NPS-Zusammensetzung", "Alle ausgewÃ¤hlten Studios")
        ...
        for code in codes_zen:
            ...
            nv = get_nps(d)
            gr = df_curr[df_curr["StudiokÃ¼rzel"] == code]["Rating"].values
            if nv is not None: nps_rows.append({"Studio": code, "NPS": nv, "Bewertung": nps_label(nv), "Google â­": round(gr[0], 2) if len(gr) > 0 else None, "Antworten": len(d)})
        if nps_rows:
...
# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
# TAB 3 Â· MANAGEMENT-BERICHT (alle Downloads hier)
# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
with tab3:
    section_title("Management-Bericht", "Automatisch aus echten Daten Â· Trend-Analyse, Insights und Handlungsempfehlungen")
    ...
    if not df_vm.empty:
        vm_r = df_vm.set_index("StudiokÃ¼rzel")["Rating"]
        for _, row in df_curr.iterrows():
            old = vm_r.get(row["StudiokÃ¼rzel"])
            if old is not None:
                dv = row["Rating"] - old
                if dv >= 0.1: verbessert.append(f"{row['StudiokÃ¼rzel']} (+{dv:.1f})")
                elif dv <= -0.1: verschlechtert.append(f"{row['StudiokÃ¼rzel']} ({dv:.1f})")
    ...
    lines = [
        ...
        s1, "",
        "I.  ÃBERBLICK", s2,
        f"  Google Ã-Rating          : {avg_rating:.2f} â­" + (f"   ({delta_r:+.2f} ggÃ¼. {vormonat})" if delta_r is not None else ""),
    ...
    if avg_rating >= RATING_GOOD: lines.append("  ð Gesamtbewertung: STARK â Region Ã¼ber Zielmarke.")
    elif avg_rating >= RATING_MIN: lines.append("  ð¡ Gesamtbewertung: SOLIDE â Einzelne Studios brauchen Aufmerksamkeit.")
    else: lines.append("  ð´ Gesamtbewertung: KRITISCH â SofortmaÃnahmen erforderlich!")

    lines += ["", "II.  TREND", s2]
    if verbessert: lines.append(f"  ð Verbessert vs. {vormonat}     : {', '.join(verbessert)}")
    if verschlechtert: lines.append(f"  ð Verschlechtert vs. {vormonat} : {', '.join(verschlechtert)}")
    if not verbessert and not verschlechtert: lines.append("  â¡  Nur ein Monat verfÃ¼gbar â kein Trendvergleich mÃ¶glich.")

    lines += ["", "III. STUDIO-STATUS", s2]
    for _, row in studios_s.iterrows():
        code = row["StudiokÃ¼rzel"]
        zs = df_zen[df_zen["Property - studio"] == code] if not df_zen.empty else pd.DataFrame()
        sn = get_nps(zs)
        nstr = f" | NPS: {sn:.0f}" if sn is not None else ""
        kstr = "  â KRITISCH!" if row["Rating"] < RATING_MIN else ""
        lines.append(f"  {rating_icon(row['Rating'])}  {code:<6} {row['Rating']:.2f} â­  (+{row['NewReviews']} Rez.){nstr}{kstr}")

    lines += ["", "IV.  HIGHLIGHTS", s2, "  ð TOP-STUDIOS:"]
    for _, row in top3.iterrows():
        lines.append(f"      â¢ {row['StudiokÃ¼rzel']} ({row['Stadt']}): {row['Rating']:.2f} â­")

    if n_crit > 0:
        lines += ["", "  ð¨ HANDLUNGSBEDARF:"]
        for _, row in crit.iterrows():
            code = row["StudiokÃ¼rzel"]
            zs = df_zen[df_zen["Property - studio"] == code] if not df_zen.empty else pd.DataFrame()
            nl = neg_labels(zs, 3)
            lines.append(f"      â¢ {code} ({row['Stadt']}): {row['Rating']:.2f} â­")
            if not nl.empty: lines.append(f"        Kundenkritik: {', '.join(nl.index.tolist())}")
        lines += ["", "  Empfohlene MaÃnahmen:", "      1.  SofortgesprÃ¤ch mit Studioleitung (diese Woche)", "      2.  Google-Rezensionen der letzten 30 Tage analysieren", "      3.  Konkrete MaÃnahmen festlegen (Frist: 2 Wochen)", "      4.  WÃ¶chentliches Follow-up einplanen"]

    if not atl.empty or not anl.empty:
        lines += ["", "V.  KUNDENFEEDBACK-INSIGHTS", s2]
        if not atl.empty:
            lines.append("  ð Meistgenannte Themen:")
            for lbl, cnt in atl.items(): lines.append(f"      â¢ {lbl} ({cnt}Ã)")
        if not anl.empty:
            lines += ["", "  â ï¸  Hauptkritikpunkte (Detraktoren):"]
            for lbl, cnt in anl.items(): lines.append(f"      â¢ {lbl} ({cnt}Ã)")
            lines.append("  â  FÃ¼r nÃ¤chstes Team-Meeting priorisieren.")

    lines += ["", s1, f"Senzera Performance Hub v5  Â·  {jetzt}", s1]
    bericht = "\n".join(lines)
    st.text_area("", value=bericht, height=540, label_visibility="collapsed")

    # ââ ALLE DOWNLOADS âââââââââââââââââââââââââââââââââââââââââââ
    st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
    section_title("Alle Downloads", "Berichte und Daten-Exporte")
    dl1, dl2 = st.columns(2)
    with dl1:
        st.download_button("ð Bericht als .txt", data=bericht.encode("utf-8"), file_name=f"Senzera_Bericht_{sel_rl}_{sel_monat}.txt", mime="text/plain", use_container_width=True)
    with dl2:
        snap = [c for c in ["StudiokÃ¼rzel", "Stadt", "Rating", "NewReviews", "TotalReviews", "NPS"] if c in df_curr.columns]
        st.download_button("ð¥ Daten-Snapshot (CSV)", data=df_curr[snap].to_csv(index=False, sep=";", decimal=",").encode("utf-8-sig"), file_name=f"Senzera_Snapshot_{sel_rl}_{sel_monat}.csv", mime="text/csv", use_container_width=True)
    dl3, dl4 = st.columns(2)
    with dl3:
        st.download_button("ð¥ Alle Google-Daten (CSV)", data=df_view.to_csv(index=False, sep=";", decimal=",").encode("utf-8-sig"), file_name=f"Senzera_Google_{sel_monat}.csv", mime="text/csv", use_container_width=True)
    with dl4:
        if not df_zen.empty:
            st.download_button("ð¥ Zenloop-Daten (CSV)", data=df_zen.to_csv(index=False, sep=";", decimal=",").encode("utf-8-sig"), file_name=f"Senzera_Zenloop_{sel_monat}.csv", mime="text/csv", use_container_width=True)
        else:
            st.button("ð¥ Zenloop â keine Daten", disabled=True, use_container_width=True)


# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
# TAB 4 Â· TEAM-PERFORMER (passwortgeschÃ¼tzt)
# ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
with tab4:
    if not CRYPTO_OK:
        st.error("â Bibliothek fehlt: `pip install cryptography`")
        st.stop()
    if not os.path.exists(PERFORMER_BIN):
        st.warning(f"â ï¸ VerschlÃ¼sselte Datei **'{PERFORMER_BIN}'** nicht gefunden.\n\nBitte zuerst das Update-Skript ausfÃ¼hren:\n```\npython performer_update.py\n```")
        st.stop()
    ...
    if not st.session_state.performer_unlocked:
        ...
        with lock_col:
            st.markdown(
                f"""<div style='text-align:center;padding:40px 32px;background:{T["card_bg"]};border:1px solid {T["card_border"]};border-radius:20px;box-shadow:{T["card_shadow"]};margin-top:24px;'>
                    ...
                    <div style='font-size:13px;color:{T["text_muted"]};margin-bottom:6px;'>GeschÃ¼tzter Bereich</div>
                    <div style='font-size:11px;color:{T["text_muted"]};padding:6px 14px;background:{T["app_bg"]};border-radius:8px;display:inline-block;margin-bottom:24px;'>ð Datei vom {file_date}</div>
                </div>""", unsafe_allow_html=True)
            st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
            pw_input = st.text_input("Passwort", type="password", placeholder="Passwort eingeben â¦", label_visibility="collapsed")
            _, btn_col, _ = st.columns([1, 2, 1])
            with btn_col: unlock_btn = st.button("ð Entsperren", use_container_width=True)
            if unlock_btn:
                if not pw_input.strip(): st.error("Bitte Passwort eingeben.")
                else:
                    with st.spinner("Wird entschlÃ¼sselt â¦"):
                        try:
                            data = _decrypt_performer(pw_input.strip())
                            st.session_state.performer_data = data
                            st.session_state.performer_unlocked = True
                            st.rerun()
                        except ValueError: st.error("â Falsches Passwort. Bitte erneut versuchen.")
                        except Exception as e: st.error(f"â Fehler beim EntschlÃ¼sseln: {e}")
    else:
        ...
        with lock_h: section_title("Team-Performer", f"Umsatz- & Krankentage-Analyse Â· Datenstand: {gen_time}")
        with lock_info:
            if st.button("ð Sperren", use_container_width=True):
                st.session_state.performer_unlocked = False
                st.session_state.performer_data = None
                st.rerun()

        if sel_rl != "Alle":
            rl_kuerzel = df_curr["StudiokÃ¼rzel"].unique().tolist()
            df_pf = df_pf_raw[df_pf_raw["KÃ¼rzel"].isin(rl_kuerzel)].copy()
            df_kr = df_kr_raw[df_kr_raw["KÃ¼rzel"].isin(rl_kuerzel)].copy()
            if df_pf.empty:
        ...
        if not avail_months: st.warning("Keine Monatsdaten verfÃ¼gbar."); st.stop()
        ...
        with pk1: _performer_kpi_card("Ã Umsatz / Person", f"{avg_umsatz:,.0f} â¬".replace(",", "."), f"Monat {pf_monat}")
        with pk2: _performer_kpi_card("Top Performer", top_row["Name"] if top_row is not None else "â", f"{top_row[pf_monat]:,.0f} â¬".replace(",", ".") if top_row is not None else "", color=C_HONEY)
        with pk3: _performer_kpi_card("Aktive MA (Vertrieb)", str(n_aktiv), f"mit Umsatz in {pf_monat}")
        with pk4: _performer_kpi_card("Ã Krankentage", f"{avg_krank:.1f} Tage", f"Gesamt {pf_monat}: {total_krank} Tage", color=C_RED if avg_krank > 3 else C_MINT)

        st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
        ch1, ch2 = st.columns([3, 2], gap="large")
        with ch1:
            section_title(f"Umsatz-Ranking â {pf_monat}", "Alle Vertriebsmitarbeiterinnen mit positivem Umsatz")
            ...
            fig_pf = go.Figure(go.Bar(x=df_rank_pf[pf_monat], y=df_rank_pf["Name"], orientation="h", marker=dict(color=bar_colors_pf, line=dict(width=0)), text=df_rank_pf[pf_monat].apply(lambda v: f"{v:,.0f} â¬".replace(",", ".")), textposition="outside", textfont=dict(color=PFG, size=11), hovertemplate="<b>%{y}</b><br>%{x:,.0f} â¬<extra></extra>"))
            fig_pf.add_vline(x=avg_umsatz, line_dash="dot", line_color=C_HONEY, line_width=2, annotation=dict(text=f"Ã {avg_umsatz:,.0f}".replace(",", "."), font=dict(size=10, color=C_HONEY), bgcolor="rgba(0,0,0,0)"))
        ...
        with ch2:
            section_title(f"Krankentage â {pf_monat}", "Top 15 nach Krankentagen")
            if kr_col and not df_kr.empty:
                ...
                else: st.info(f"Keine Krankentage im {pf_monat} verzeichnet. â")
            else: st.caption("Keine Krankentage-Daten fÃ¼r diesen Monat.")

        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        section_title("Monatstrend", "Ã Umsatz pro Monat")
        trend_data = []
        for m in avail_months:
            sub = df_pf_raw[df_pf_raw[m].notna() & (df_pf_raw[m] > 0) & df_pf_raw["Position"].isin(["KM", "SL", "Standortleitung"])]
            if not sub.empty: trend_data.append({"Monat": m, "Ã Umsatz": sub[m].mean(), "Anz. MA": len(sub), "Ã Krank": df_kr_raw[m].mean() if m in df_kr_raw.columns else 0})
        if trend_data:
            df_trend = pd.DataFrame(trend_data)
            fig_trend = go.Figure()
            fig_trend.add_trace(go.Scatter(x=df_trend["Monat"], y=df_trend["Ã Umsatz"], fill="tozeroy", fillcolor="rgba(232,98,10,0.06)", line=dict(width=0), showlegend=False, hoverinfo="skip"))
            fig_trend.add_trace(go.Scatter(x=df_trend["Monat"], y=df_trend["Ã Umsatz"], name="Ã Umsatz", line=dict(color=C_ORANGE, width=2.5), marker=dict(size=8, color=C_ORANGE, line=dict(width=2.5, color=T["app_bg"])), mode="lines+markers", hovertemplate="<b>%{x}</b><br>Ã Umsatz: %{y:,.0f} â¬<extra></extra>"))
            if "Ã Krank" in df_trend.columns:
                fig_trend.add_trace(go.Scatter(x=df_trend["Monat"], y=df_trend["Ã Krank"], name="Ã Krankentage", line=dict(color=C_RED, width=2, dash="dot"), marker=dict(size=6, color=C_RED), mode="lines+markers", yaxis="y2", hovertemplate="<b>%{x}</b><br>Ã Krank: %{y:.1f} Tage<extra></extra>"))
            fig_trend.update_layout(plot_bgcolor=PBG, paper_bgcolor=PBG, font=dict(color=PFG, family="Plus Jakarta Sans", size=12), showlegend=True, legend=dict(orientation="h", yanchor="bottom", y=1.02, font=dict(size=12)), yaxis=dict(title="Ã Umsatz (â¬)", gridcolor=PGRD, zeroline=False, tickformat=",.0f", title_font=dict(size=11, color=T["text_muted"])), yaxis2=dict(title="Ã Krankentage", overlaying="y", side="right", gridcolor="rgba(0,0,0,0)", zeroline=False, title_font=dict(size=11, color=C_RED), tickfont=dict(color=C_RED)), xaxis=dict(gridcolor=PGRD, zeroline=False), height=300, margin=dict(l=4, r=60, t=30, b=8))
            st.plotly_chart(fig_trend, use_container_width=True)

        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        section_title("Detailtabelle", f"Alle Vertriebsmitarbeiterinnen Â· Monat {pf_monat}")
        ...
        df_detail = df_detail.rename(columns={pf_monat: "Umsatz (â¬)"})
        df_detail = df_detail.sort_values("Umsatz (â¬)", ascending=False, na_position="last")
        df_detail["Umsatz (â¬)"] = df_detail["Umsatz (â¬)"].apply(lambda v: f"{v:,.0f}".replace(",", ".") if pd.notna(v) else "â")
...
st.markdown(
    f"""<div style='display:flex;align-items:center;justify-content:space-between;padding:28px 0 16px;'>
        <div>
            ...
            <div style='font-size:10px;color:{T["text_muted"]};letter-spacing:0.3px;'>v5 Â· waxing & beauty</div>
        </div>
        <div style='text-align:right;'>
            ...
            <div style='font-size:11px;color:{C_ORANGE};font-weight:600;'>Alle Daten lokal Â· Keine Ãbertragung</div>
        </div>
    </div>""", unsafe_allow_html=True)
